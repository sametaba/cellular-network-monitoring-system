...
def format_ascii_box(design_system: dict) -> str:
    """Format design system as box output."""
    ...
    lines.append(f"  TARGET: {project} - RECOMMENDED DESIGN SYSTEM")
    lines.append("=" * BOX_WIDTH)
    lines.append("-" * BOX_WIDTH)
    ...
    for label, key, css_var in color_entries:
        ...
        if not hex_val:
            continue
        content = f"|     {label + ':':14s} {hex_val:10s} ({css_var})"
        lines.append(content.ljust(BOX_WIDTH) + "|")
    if colors.get("notes"):
...
def format_master_md(design_system: dict) -> str:
    ...
    lines.append("  transition: all 200ms ease;")
    lines.append("}")
    lines.append(".card:hover { box-shadow: var(--shadow-lg); transform: translateY(-2px); }")
    lines.append("```")
    lines.append("")
...
def _generate_intelligent_overrides(page_name: str, page_query: str, design_system: dict) -> dict:
    """Generate intelligent overrides based on page type."""
